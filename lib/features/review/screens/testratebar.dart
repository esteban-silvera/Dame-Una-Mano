...
class RateWorkerFunction extends StatelessWidget {
  ...
  void _saveRatingToFirestore(String userId, String workerId, double rating) {
    ...
    FirebaseFirestore.instance.collection('Users').doc(userId).collection('ratings').add({
      'workerId': workerId,
      'rating': rating,
    }).then((value) {
      ...
      FirebaseFirestore.instance.collection('professionals').doc(workerId).collection('ratings').add({
        'userId': userId,
        'rating': rating,
      }).then((value) {
        print('Calificación guardada exitosamente en Firestore del trabajador');
      }).catchError((error) {
        print('Error al guardar la calificación en Firestore del trabajador: $error');
      });

    }).catchError((error) {
      print('Error al guardar la calificación en Firestore del usuario: $error');
    });
  }
}
